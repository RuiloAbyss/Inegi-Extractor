...
from processors.cluster_calculator import calculate_clusters # NUEVA IMPORTACIÓN

class MainWindow:
    def __init__(self, root):
        self.root = root
        self.root.geometry("1250x800")
        self.root.title("INEGI Data Processor - Análisis de Clusters")
        ...
        self.df_calc_cache = None # Memoria para los cálculos
        
        self.current_n_population = 0 # Variable global N (Población en Rango)
    ...
    def setup_ui(self):
        ...
        # 3. FILTRO GEOGRÁFICO Y ANÁLISIS
    ...
    def prepare_environment(self):
        ...
        # Pestaña de Cálculos se activa solo si ambos están o si solo está el económico (asumimos N=1)
        ...
        messagebox.showinfo("Entorno Listo", "Listos para calcular. Selecciona filtros y Analiza.")
    ...
    # ---------------------------------------------------------
    # CONSTRUCCIÓN DE PESTAÑAS E INTERFACES (Frontend)
    # ---------------------------------------------------------
    ...
    def build_pop_tab(self):
        ...
        tk.Label(filter_frame, text="Filtro Edad:").pack(side=tk.LEFT, padx=5)
        ...
        self.lbl_total_filtro = tk.Label(filter_frame, text="N (Población en rango): -", font=("Arial", 9, "bold"))
    ...
    def build_econ_tab(self):
        ...
        tk.Label(row1_frame, text="Año Censal:").pack(side=tk.LEFT, padx=5)
    ...
    def build_calc_tab(self):
        """Construye la nueva pestaña de Cálculos de Clusters."""
        ...
        self.chk_clusters_only.pack(side=tk.LEFT, padx=20)
    ...
    def process_all_tabs(self):
        ...
        if self.df_econ_raw is not None: 
            ...
            self.process_calculations() # Dispara el nuevo motor matemático
    ...
    def process_population(self):
        try:
            ...
            # Recolectar la Variable 'N' para enviarla a los Clusters
            t_flt = sum(pd.to_numeric(str(x).replace(',', ''), errors='coerce') or 0 for col in age_cols for x in state_data[col].values)
            self.current_n_population = t_flt
            self.lbl_total_filtro.config(text=f"N (Población en rango): {t_flt:,.0f}")
            
            res_df = calculate_population_differences(state_data, mun_data, age_cols)
            self.render_tree(self.tree_pop, res_df, type_tab='pop')
        except Exception as e:
    ...
    def process_calculations(self):
        """Dispara las matemáticas de Clusters con el censo económico y la N de población."""
        try:
            if hasattr(self, 'txt_calc_search'): self.txt_calc_search.delete(0, tk.END)
            # Llama al nuevo procesador matemático en cascada
            self.df_calc_cache = calculate_clusters(self.df_econ_cache, self.current_n_population)
    ...
    def filter_economic_realtime(self, event=None):
        ...
        if hasattr(self, 'cb_sort_col') and self.cb_sort_col.winfo_exists():
            ...
            if sort_val != "Jerarquía Original":
                col_map = {"Unidades Econ.": "Unidades Econ.", "Personal Ocupado": "Personal Ocupado"}
                col = col_map.get(sort_val)
    ...
    def filter_calc_realtime(self, event=None):
        """Filtra y dibuja la tabla de Clusters en vivo."""
        ...
        # Filtro de texto (Busca en la clave del hijo o en el nombre de ambos)
        if search != "":
            filtered = filtered[
    ...
    def render_tree(self, tree, df, type_tab='pop'):
        ...
        for col in display_df.columns:
            # Anchuras predeterminadas para los cálculos para máxima limpieza visual
            if type_tab == 'calc':
                ...
                if type_tab == 'pop': w = (max_len * 8) + 12 if col == 'Municipio' else (max_len * 7) + 8
        ...
        for _, row in display_df.iterrows():
            ...
            for col, item in zip(display_df.columns, list(row)):
                if col == '¿Es Cluster?' and item == 'Sí': is_cluster = True
                
                # Formateo de números decimales para los coeficientes
                if type_tab == 'calc' and col in ['Kc', 'Ks', 'Ki']:
                    vals.append(f"{item:,.3f}")
                elif isinstance(item, float):
                    vals.append(f"{item:,.0f}")
                else:
                    vals.append(str(item))
                    
            tags = ('cluster_row',) if is_cluster else ()
